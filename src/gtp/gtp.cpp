  ...
  */
  string takeRequest(string request) {
    ...
    try
    {
      ...
      if(id <= 0)
      {
        return "?"+to_string(id)+" Invalid Argument: id must be a positive integer\n\n";
      }
    }
  ...
  if(given_args != required_args)
  {
    return "?"+id_+"Invalid Argument: "+ command + " arguments list should be " + to_string(required_args) + ", however, " + to_string(given_args) + " was provided\n\n";
  }
  ...
  else return "?"+id_+"command doesn't exist\n\n";
